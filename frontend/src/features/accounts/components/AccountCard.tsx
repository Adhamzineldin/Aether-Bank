...
import type { AccountStatus } from '@veld/types';
...
export function AccountCard({ data }: { data: Account }) {
  return (
    <Link to={ROUTES.account(data.id)} className="block group">
      <Card className="p-5 transition-all group-hover:border-primary/50 group-hover:shadow-soft">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-wider text-muted-fg">{data.accountType}</p>
            <p className="mt-1 font-mono text-sm">{maskAccount(data.accountNumber)}</p>
          </div>
          <Badge tone={statusTone[data.status]} className="gap-1">
        ...
        <div className="mt-6">
          ...
          <p className="mt-0.5 text-2xl font-bold">
            <CurrencyDisplay amount={data.balance} currency={data.currency} />
          </p>
        </div>
      </Card>
    </Link>
