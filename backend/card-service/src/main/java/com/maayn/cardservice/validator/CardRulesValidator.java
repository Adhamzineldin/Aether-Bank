...
public class CardRulesValidator {
    ...
    public void validateCardForPayment(Card card, String currency, BigDecimal amount) throws ProcessMerchantPaymentException {
        ...
        if (!normalizeCurrency(currency).matches("[A-Z]{3}")) {
            throw CardErrors.processMerchantPayment.invalidCurrency("Currency must be a 3-letter ISO code");
        }
        if (card.getCardType() == CardType.CREDIT) {
            validateCreditLimit(card.getCreditDetails(), amount);
        }
    }
    ...
    private void validateCreditLimit(CreditCardDetailsEntity creditDetails, BigDecimal amount) throws ProcessMerchantPaymentException {
