...
public final class CardMapper {
    ...
    public static CardSummary toCardSummary(Card card) {
        return new CardSummary(
                ...
                card.getCardToken(), card.getLastFourDigits(),
                card.getCardType(), card.getCardNetwork(), card.getStatus(),
                card.getExpiryMonth(), card.getExpiryYear(),
                card.getIssuedAt(), card.getActivatedAt(), card.getBlockedAt(), card.getBlockReason());
    }

    public static CreditCardDetails toCreditCardDetails(CreditCardDetailsEntity entity) {
        if (entity == null) return null;
        return new CreditCardDetails(
                entity.getCreditLimit(), entity.getAvailableCredit(), entity.getCurrentBalance(),
                entity.getMinimumPayment(), entity.getPaymentDueDate(),
                entity.getAnnualInterestRate(), entity.getBillingCycleDay(), entity.getLastStatementDate());
    }

    public static CardTransactionResponse toTransactionResponse(CardTransaction entity) {
        return new CardTransactionResponse(
                entity.getId(), entity.getCard().getId(), entity.getMerchantId(),
                entity.getAuthCode(), null,
                entity.getAmount(), entity.getCurrency(),
                entity.getAmountInBaseCurrency(), entity.getExchangeRate(),
                entity.getStatus(), entity.getType(), entity.getProcessedAt());
    }
