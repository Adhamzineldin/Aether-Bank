package com.maayn.notificationservice.audit;
...
/**
 * Lightweight audit-event publisher for notification-service. Mirrors the
 * publishers in card/account/financial/iam services — emits a JSON map to the
 * central {@code security_audit_exchange} so the audit-service captures
 * notification-side events (email send success/failure, channel adapter
 * fallbacks, retry exhaustion) alongside everything else.
 */
@Component
